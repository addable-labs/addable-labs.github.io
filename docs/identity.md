...
- **Page hero, article, blog, 404.** Inner pages open with the landing
  ...
  centred composition (founder feedback 2026-09-22, replacing the
  left-edge measure and figure lane of si-55iu): header, footer and every
  text block sit on one 44 rem measure (`--measure`, about 80 characters
  at 1.0625 rem / 1.7) centred in the container, and the page gets its
  width from the wide figures and the closing "More from the blog" band
...
- **Article figures** (`.figure`, `src/_includes/figures/figures.mjs`, the
  ...
  hairlines for scaffolding; every word from `strings.figures.<id>`. Both
  placements sit in the flow between the paragraphs: a wide figure spans
  the whole container with its panels in one row (`auto-fit`, stacking
  below 48 rem) and its caption centred underneath on the measure; an
  inline figure (one panel) sits on the measure and from 48 rem is a grid
  of the panel (`minmax(20rem, 24.5rem)`, where it renders between 1:1 and
  1.23:1) and the caption beside it, top-aligned — below 48 rem the
  caption is under the panel. The layout gate measures all of it on every
  article page at five widths.
